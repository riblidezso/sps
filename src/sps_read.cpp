...
//
int sps_read::read_model_bin_all_cont()
{
	#define NO_METALL 6
...
//Reading all models from binary file 
//
int sps_read::read_model_bin_all()
{
	#define NO_METALL 6
	#define PATH "../input/bin/"
...
//
int sps_read::read_model_bin(int i, std::string infilename)
{
...
//Reading one model from binary file 
//
int sps_read::read_model_bin_cont( std::string infilename)
{
	std::ifstream infile(infilename.c_str(), std::ios::binary |std::ios::ate);
	//	CHECKING FILENAME
	if(!(infile))
...
int sps_read::usr_read_sdss_csv()
{
	bool tryagain;	
